...
- 初回のみ `python3 scripts/notion_add_theme.py` を実行して `テーマ` プロパティ追加と既存ページの遡及要約を行う。**次の月曜09:00の週次自動実行より前に必ず実行すること**：`テーマ` プロパティが存在しないと、月曜のページ作成がNotion APIの400エラーで失敗し、そのままサイレントに失敗する（例外は出ない）。その週のコンテンツはNotionに一切保存されず、ローカルの `logs/` にのみ残る。
